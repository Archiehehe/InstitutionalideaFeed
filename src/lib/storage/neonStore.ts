...
function getClient() {
  ...
  if (!databaseUrl) {
    throw new Error("Storage is not configured. Set DATABASE_URL in Vercel.");
  }
...
export function createNeonStore(): Store {
  const store: Store = {
    ...
    async getSourceByDomain(domain: string) {
      return maybeOne("select * from sources where lower(domain) = lower($1)", [
        domain,
      ]);
    },
    ...
    async getArticleByUrl(url) {
      return maybeOne(
        "select * from articles where url = $1 or canonical_url = $1 limit 1",
        [url],
      );
    },
    ...
    async getExtraction(articleId) {
      return maybeOne(
        "select * from article_extractions where article_id = $1",
        [articleId],
      );
    },
    ...
    async deleteBasket(id) {
      const rows = await getClient().query(
        "delete from baskets where id = $1 returning id",
        [id],
      );
      return rows.length > 0;
    },
    ...
    async getBasketMembers(basketId) {
      return many("select * from basket_members where basket_id = $1", [
        basketId,
      ]);
    },
    async addBasketMember(input) {
      ...
      const [query, params] = insertQuery("basket_members", {
        ...input,
    ...
    async removeBasketMember(id) {
      const rows = await getClient().query(
        "delete from basket_members where id = $1 returning id",
        [id],
      );
      return rows.length > 0;
    },
    ...
    async getWatchlistItem(ticker) {
      return maybeOne("select * from watchlist where ticker = upper($1)", [
        ticker,
      ]);
    },
    async addWatchlistItem(input) {
      const existing = await maybeOne<WatchlistItem>(
        "select * from watchlist where ticker = upper($1)",
        [input.ticker],
      );
      if (existing) return existing;

      const [query, params] = insertQuery("watchlist", {
        ...input,
        ticker: input.ticker.toUpperCase(),
      } as Row);
      return one(query, params);
    ...
    async getScanRuns(limit = 10) {
      return many("select * from scan_runs order by started_at desc limit $1", [
        limit,
      ]);
    },
    ...
    async getScanUrlResults(scanRunId) {
      return many(
        "select * from scan_url_results where scan_run_id = $1 order by created_at",
        [scanRunId],
      );
    },
    ...
    async getConvictionLists() {
      return many(
        "select * from conviction_lists order by updated_at desc, display_name asc",
      );
    },
    async getConvictionList(id) {
      return maybeOne(
        "select * from conviction_lists where id = $1 or slug = $1 limit 1",
        [id],
      );
    },
    ...
    async addConvictionListMember(input) {
      ...
      const [query, params] = insertQuery("conviction_list_members", {
        ...input,
        ticker: input.ticker.toUpperCase(),
      } as Row);
      return one(query, params);
    ...
    async getManagerHoldingsCount() {
      const rows = await getClient().query(
        "select count(*)::int as count from manager_holdings",
        [],
      );
    ...
    async get13FOverlapsForTickers(tickers) {
      ...
      const byManager = new Map<
        string,
        ThirteenFOverlap & { _weights: number[] }
      >();
      for (const row of rows as Row[]) {
        ...
        if (!existing) {
          byManager.set(managerId, {
            ...
            whalewisdomUrl: row.whalewisdom_url
              ? String(row.whalewisdom_url)
              : undefined,
            filingPeriod: row.filing_period
              ? String(row.filing_period)
              : undefined,
            overlapCount: 1,
        ...
        if (action) {
          ...
          existing.actionSummary[action] =
            (existing.actionSummary[action] ?? 0) + 1;
        }
      }
      ...
      return Array.from(byManager.values())
        ...
        .map(({ _weights, ...overlap }) => ({
          ...overlap,
          matchedManagerWeight:
            _weights.length > 0
              ? Number(
                  _weights.reduce((sum, value) => sum + value, 0).toFixed(2),
                )
              : undefined,
        }))
        .sort(
