...
function InstitutionBreakdown({ lists }: { lists: OverlapListEntry[] }) {
  ...
  const entries = Array.from(byInst.entries()).sort(
...
function SignalSummary({ row }: { row: OverlapRow }) {
  const {
    topThemes,
    topSectors,
    mentionedByInstitutionsCount,
    verifiedMentions,
    needsReviewMentions,
  } = useMemo(() => {
    const lists = row.lists;
    const mentionedByInstitutions = Array.from(
      new Set(lists.map((l) => l.institution)),
    ).filter(Boolean);

    const verifiedMentions = lists.filter(
      (l) => l.confidence === "verified",
    ).length;
    const needsReviewMentions = lists.filter(
    ...
    for (const l of lists) {
      if (l.theme)
        themeCounts.set(l.theme, (themeCounts.get(l.theme) ?? 0) + 1);
      if (l.sector)
    ...
    <div className="space-y-3">
      <div className="rounded-md border border-[#1F1F1F] bg-[#0A0A0A] p-3 text-xs">
        <div className="text-muted-foreground">
          Mentioned by {mentionedByInstitutionsCount} institutions across{" "}
          {row.listCount} lists
        </div>
        <div className="mt-1">Verified mentions: {verifiedMentions}</div>
        <div className="mt-0.5">
          Needs-review mentions: {needsReviewMentions}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        <div className="rounded-md border border-[#1F1F1F] bg-[#0A0A0A] p-3 text-xs">
          ...
          <div className="mt-1 font-medium">
            {topThemes.length ? topThemes.join(", ") : "—"}
          </div>
        </div>
        <div className="rounded-md border border-[#1F1F1F] bg-[#0A0A0A] p-3 text-xs">
...
export function ConvictionListOverlapPageContent() {
  ...
  // Drawer selection
  const [selectedTickerForDrawer, setSelectedTickerForDrawer] = useState<
    string | null
  >(null);

  // Multi-selection
  const [selectedTickers, setSelectedTickers] = useState<
    Map<string, SelectedTickerMeta>
  >(new Map());
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/conviction-lists/overlap?${query.toString()}`,
        );
        if (!res.ok) throw new Error("Failed to load overlap data");
        setData(await res.json());
      } catch (err) {
        setError(
          err instanceof Error ? err.message : "Failed to load overlap data",
        );
      } finally {
        setLoading(false);
      }
  ...
  const institutions = useMemo(
    () =>
      Array.from(new Set(safeRows.flatMap((row) => row.institutions))).sort(),
    [safeRows],
  );
  const years = useMemo(
    () =>
  ...
  const themes = useMemo(
    () => Array.from(new Set(safeRows.flatMap((row) => row.themes))).sort(),
    [safeRows],
  );
  const sectors = useMemo(
    () => Array.from(new Set(safeRows.flatMap((row) => row.sectors))).sort(),
    [safeRows],
  );
  ...
  const sourceTypes = useMemo(
    () =>
      Array.from(
        new Set(
          safeRows.flatMap((row) => row.lists.map((list) => list.sourceType)),
        ),
      ).sort(),
    [safeRows],
  );
  ...
  const isSelected = (ticker: string) =>
    selectedTickers.has(normalizeClipboardText(ticker));

  const copyTickers = async (tickers: string[]) => {
    const normalized = Array.from(
      new Set(tickers.map(normalizeClipboardText)),
    ).filter(Boolean);
    try {
      await navigator.clipboard.writeText(normalized.join(", "));
      setCopyFeedback(
        `Copied ${normalized.length} ticker${normalized.length === 1 ? "" : "s"}`,
      );
  ...
  const dedupedSelectedTickers = useMemo(
    () => Array.from(selectedTickers.keys()),
    [selectedTickers],
  );
  ...
  const selectedRowTickersInView = useMemo(
    () => safeRows.map((r) => r.ticker),
    [safeRows],
  );
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Find stocks repeatedly appearing across sell-side lists.
            </p>
          </div>
          <Link href="/conviction-lists">
            <Button variant="outline" size="sm">
              Back to Conviction Lists
            </Button>
          </Link>
        </div>
        <EmptyState
          ...
          actions={[
            {
              label: "Back to Conviction Lists",
              onClick: () => router.push("/conviction-lists"),
            },
          ]}
        />
      </div>
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold">Conviction List Overlap</h1>
          <p className="text-sm text-muted-foreground">
            Find stocks repeatedly appearing across sell-side lists. Mentions
            are discovery signals, not recommendations by themselves.
          </p>
        </div>
        <Link href="/conviction-lists">
          <Button variant="outline" size="sm">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Total tickers</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-semibold">
            {data.summary.totalTickers}
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">2+ list mentions</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-semibold">
            {data.summary.repeatedTickerCount}
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Verified-list mentions</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-semibold">
            {data.summary.verifiedMentions}
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Needs-review mentions</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-semibold">
            {data.summary.needsReviewMentions}
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              handleSelectChange("year", value === "all" ? "" : value)
            }
          >
            <SelectTrigger>
              <SelectValue placeholder="All" />
        ...
        <div className="space-y-2">
          ...
          <Select
            value={filters.minMentions || "1"}
            onValueChange={(value) => handleSelectChange("minMentions", value)}
          >
            <SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              handleSelectChange("sector", value === "all" ? "" : value)
            }
          >
            <SelectTrigger>
              <SelectValue placeholder="All" />
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              handleSelectChange("theme", value === "all" ? "" : value)
            }
          >
            <SelectTrigger>
              <SelectValue placeholder="All" />
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              handleSelectChange("reviewStatus", value === "all" ? "" : value)
            }
          >
            <SelectTrigger>
              <SelectValue placeholder="All" />
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              handleSelectChange("confidence", value === "all" ? "" : value)
            }
          >
            <SelectTrigger>
              <SelectValue placeholder="All" />
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              handleSelectChange("sourceType", value === "all" ? "" : value)
            }
          >
            <SelectTrigger>
              <SelectValue placeholder="All" />
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button
          variant="outline"
          size="sm"
          onClick={() => smartSelect((r) => r.mentionCount >= 2)}
        >
          Select 2+ mentions
        </Button>
        <Button
          variant="outline"
          size="sm"
          onClick={() => smartSelect((r) => r.institutionCount >= 2)}
        >
        ...
        <Button
          ...
          size="sm"
          onClick={() =>
            smartSelect((r) => r.lists.some((l) => l.confidence === "verified"))
          }
        >
        ...
        <Button
          ...
          onClick={() =>
            smartSelect((r) =>
              r.lists.some((l) => l.confidence === "needs_review"),
            )
          }
        >
      ...
      {selectedCount > 0 && (
        <div className="sticky top-0 z-40 -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 bg-[#0A0A0A] border-b border-[#1F1F1F]">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                size="sm"
                className="gap-1"
                onClick={() => copyTickers(dedupedSelectedTickers)}
              >
          ...
          {copyFeedback && (
            <div className="mt-2 text-xs text-blue-300">{copyFeedback}</div>
          )}
          {basketFeedback && (
            <div className="mt-2 text-xs text-blue-300">{basketFeedback}</div>
          )}
        </div>
      )}
                  ...
                  onChange={(e) => {
                    ...
                    if (checked) selectAllVisible();
                      ...
                      const visible = new Set(
                        safeRows.map((r) => normalizeClipboardText(r.ticker)),
                      );
                      setSelectedTickers((prev) => {
                        const next = new Map(prev);
      ...
      {selectedTickerRow && (
        <div className="fixed inset-0 z-50">
          <div
            ...
            onClick={() => setSelectedTickerForDrawer(null)}
          />
          <div className="absolute right-0 top-0 h-full w-full max-w-3xl overflow-y-auto bg-[#0A0A0A] border-l border-[#1F1F1F]">
            <div className="p-4 sm:p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-lg font-semibold">
                    {selectedTickerRow.ticker}
                  </h2>
                  <p className="text-sm text-muted-foreground">
                    {selectedTickerRow.companyName ?? "—"}
                  </p>
                </div>
                <Button
                  variant="ghost"
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <Stat
                  label="Mention count"
                  value={String(selectedTickerRow.mentionCount)}
                />
                <Stat
                  label="Institution count"
                  value={String(selectedTickerRow.institutionCount)}
                />
                <Stat
                  label="Latest"
              ...
              <div className="mt-5 rounded-md border border-[#1F1F1F] bg-[#0A0A0A] p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                    ...
                    Use selections to turn repeated sell-side mentions into a
                    research basket. Mentions are discovery signals, not
                    recommendations.
                  </div>
                  <div className="flex flex-wrap gap-2">
                      ...
                      onClick={() => {
                        setSelectedTickers((prev) => {
                          ...
                          next.set(selectedTickerRow.ticker, {
                            ...
                            institutionCount:
                              selectedTickerRow.institutionCount,
                    ...
                    >
                      <FolderPlus className="h-3.5 w-3.5" /> Add this ticker to
                      Watchlist
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
                      className="gap-1"
                      onClick={() => copyTickers([selectedTickerRow.ticker])}
                    >
              ...
              <div className="mt-5 space-y-5">
                          ...
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              ...
                              <div className="mt-1 grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                                <FieldLine
                                  ...
                                  value={
                                    entry.year
                                      ? `${entry.year}${entry.period ? ` ${entry.period}` : ""}`
                                      : (entry.period ?? "—")
                                  }
                                />
                                <FieldLine
                                ...
                                <FieldLine
                                  label="Confidence"
                                  value={entry.confidence.replace("_", " ")}
                                />
                                <FieldLine
                                  label="Source type"
                                  value={entry.sourceType.replace("_", " ")}
                                />
                                <FieldLine
                                  label="Member action"
                                  value={entry.action ?? "—"}
                            ...
                            <div className="shrink-0">
                              <Button
                                ...
                                onClick={(e) => e.stopPropagation()}
                              >
                                <Link href={`/conviction-lists/${entry.id}`}>
                                  Open
                                </Link>
                              </Button>
                            </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="text-xs text-muted-foreground">
                    Most common themes/sectors are derived from the current
                    result set.
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button
                      ...
                      onClick={() =>
                        router.push(
                          `/conviction-lists?search=${selectedTickerRow.ticker}`,
                        )
                      }
                    >
                      <Search className="h-3.5 w-3.5" /> Open related conviction
                      lists
                    </Button>
                    <Button
                      variant="outline"
