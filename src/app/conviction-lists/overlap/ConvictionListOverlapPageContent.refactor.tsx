...
function SignalSummary({ row }: { row: OverlapRow }) {
  const { topThemes, topSectors, mentionedByInstitutionsCount, verifiedMentions, needsReviewMentions } = useMemo(() => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Find stocks repeatedly appearing across sell-side lists. Repeated mentions are not recommendations by themselves; use this as a research discovery layer.
          </p>
        </div>
            ...
            <div className="p-4 sm:p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{selectedRow.companyName ?? '—'}</p>
                </div>
...
export function ConvictionListOverlapPageContent() {
  ...
  const years = useMemo(
    () => Array.from(new Set(safeRows.flatMap((row) => row.lists.map((list) => list.year).filter((y): y is number => y !== undefined)))).sort((a, b) => b - a),
    [safeRows],
  )
  ...
  const reviewStatuses = useMemo(() => Array.from(new Set(safeRows.flatMap((row) => row.reviewStatuses))).sort(), [safeRows])
  const confidences = useMemo(() => Array.from(new Set(safeRows.flatMap((row) => row.confidences))).sort(), [safeRows])
  const sourceTypes = useMemo(() => Array.from(new Set(safeRows.flatMap((row) => row.lists.map((list) => list.sourceType)))).sort(), [safeRows])
