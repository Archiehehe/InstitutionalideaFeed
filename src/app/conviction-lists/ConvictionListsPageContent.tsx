...
// ... interfaces ConvictionListData, Diagnostics, PageData, PasteParseResult, QueryData, ImportResult ...
...
export function ConvictionListsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/conviction-lists?${searchParams.toString()}`,
      );
      if (!res.ok) {
        ...
        throw new Error(data.error || "Failed to load conviction lists");
      }
      setData(await res.json());
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to load conviction lists",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleImportSeed = async () => {
    ...
    const res = await fetch("/api/conviction-lists/import-seed", {
      method: "POST",
    });
  ...
  const handleReviewAction = async (
    listId: string,
    action: "verified" | "needs_review" | "rejected",
  ) => {
    try {
  ...
  const institutions = Array.from(
    new Set(data.lists.map((l) => l.institution)),
  ).sort();
  const years = Array.from(
    new Set(
      data.lists.map((l) => l.year).filter((y): y is number => y !== undefined),
    ),
  ).sort((a, b) => b - a);
    ...
    <div className="space-y-5">
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={pasteDialogOpen} onOpenChange={setPasteDialogOpen}>
            ...
            <DialogContent className="max-w-xl">
              <DialogHeader>
                <DialogTitle>Paste List</DialogTitle>
              </DialogHeader>
              <p className="text-xs text-muted-foreground">
                Format: header line with institution and list name, then ticker
                lines (TICKER - Company Name).
              </p>
              <Textarea
                value={pasteText}
                onChange={(e) => setPasteText(e.target.value)}
                rows={8}
                placeholder="BofA Fab Five Software Stocks for H2 2026&#10;SNOW - Snowflake&#10;DDOG - Datadog&#10;FROG - JFrog&#10;MDB - MongoDB&#10;TWLO - Twilio"
              />
              ...
              {parsedPaste && (
                <div className="space-y-2 rounded border border-[#1F1F1F] bg-[#0A0A0A] p-3">
                  <p className="text-xs font-medium">
                    {parsedPaste.institution} - {parsedPaste.listName}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {parsedPaste.members.length} tickers parsed
                  </p>
                  ...
                  <Button size="sm" className="mt-2" onClick={handleSaveParsed}>
                    Save List
                  </Button>
                </div>
              )}
          ...
          <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                <DialogTitle>Add Conviction List</DialogTitle>
              </DialogHeader>
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Field
                  ...
                  onChange={(v) => setFormData({ ...formData, listName: v })}
                />
                <Field
                  label="Year"
                  value={formData.year}
              ...
              <Field
                ...
                onChange={(v) => setFormData({ ...formData, sourceUrl: v })}
              />
              <div>
                <label className="text-xs text-muted-foreground">
            ...
            <details>
              ...
              <div className="mt-2 space-y-2 text-xs">
                {importResult.errors.map((item, i) => (
                  <div
                    key={i}
                    className="rounded border border-red-800 bg-red-900/20 p-2"
                  >
                    <div className="font-medium">
                      {item.institution} - {item.listName}
                    </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 p-4 rounded-md border border-[#1F1F1F] bg-[#0A0A0A]">
        ...
        <Select
          ...
          onValueChange={(v) => setFilter("institution", v || "")}
        >
          <SelectTrigger className="h-9 text-xs">
        ...
        <Select
          value={filters.theme || ""}
          onValueChange={(v) => setFilter("theme", v || "")}
        >
          <SelectTrigger className="h-9 text-xs">
        ...
        <Select
          ...
          onValueChange={(v) => setFilter("reviewStatus", v || "")}
        >
          <SelectTrigger className="h-9 text-xs">
        ...
        <Select
          ...
          onValueChange={(v) => setFilter("sourceType", v || "")}
        >
          <SelectTrigger className="h-9 text-xs">
      ...
      <div className="grid gap-2 md:grid-cols-5">
        <SummaryCell
          ...
          value={String(diagnostics.totalLists)}
        />
        <SummaryCell label="Verified" value={String(diagnostics.verified)} />
        <SummaryCell
          label="Needs Review"
          value={String(diagnostics.needsReview)}
        />
        <SummaryCell
          label="Seed Available"
          value={String(diagnostics.seedAvailable)}
        />
        <SummaryCell
          label="Partial Candidates"
          value={String(diagnostics.partialCandidates)}
        />
      </div>
      ...
      {lists.length === 0 ? (
        <EmptyState
          ...
          description="Import starter lists or upload/add lists to begin tracking high-conviction institutional ideas."
          actions={[
      ...
      <Dialog open={queriesDialogOpen} onOpenChange={setQueriesDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Sell-Side List Finder Queries</DialogTitle>
          </DialogHeader>
          {queryWindow && (
            <p className="text-xs text-muted-foreground">
              Window: {new Date(queryWindow.fromDate).toLocaleDateString()} →{" "}
              {new Date(queryWindow.toDate).toLocaleDateString()} (year{" "}
              {queryWindow.yearLabel})
            </p>
          )}
          {queryWindow && (
            <p className="text-xs text-muted-foreground">
              Source-type explanation: Official bank pages are verified
              evidence. Media summaries are allowed as list evidence but start
              as needs_review.
            </p>
          )}
          <p className="text-xs text-muted-foreground">
          ...
          <div className="max-h-96 space-y-1 overflow-y-auto">
            {queries.map((q, i) => (
              <div
                key={i}
                className="flex items-center justify-between rounded border border-[#1F1F1F] px-3 py-1.5 text-xs"
              >
                <span>{q.query}</span>
      ...
      <Card key={list.id} className="border border-[#1F1F1F] bg-[#0A0A0A]">
        <CardContent className="space-y-3 p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <Link
                href={`/conviction-lists/${list.id}`}
                className="text-sm font-semibold text-[#E5E7EB] hover:underline"
              >
                {list.displayName}
              </Link>
            ...
            <div className="flex flex-wrap justify-end gap-1">
              <Badge variant="outline" className="text-xs capitalize">
                {list.confidence.replace("_", " ")}
              </Badge>
              {list.reviewStatus && (
                <Badge variant="outline" className="text-xs capitalize">
                  {list.reviewStatus.replace("_", " ")}
                </Badge>
              )}
              <Badge variant="secondary" className="text-xs">
                {list.tickerCount} tickers
              </Badge>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[#9CA3AF]">
            ...
            {list.sourceType && (
              <span>Source: {list.sourceType.replace("_", " ")}</span>
            )}
            {list.sourcePublisher && (
              <span>Publisher: {list.sourcePublisher}</span>
            )}
            <span>
          ...
          <div className="flex flex-wrap gap-2 border-t border-[#1F1F1F] pt-2">
            <Button
              ...
              onClick={() => onReviewAction(list.id, "verified")}
            >
            ...
            <Button
              ...
              size="sm"
              className="h-7 gap-1 text-xs"
              onClick={() => onReviewAction(list.id, "needs_review")}
            >
            ...
            <Button
              ...
              size="sm"
              className="h-7 gap-1 text-xs"
              onClick={() => onReviewAction(list.id, "rejected")}
            >
            ...
            <div className="ml-auto flex gap-2">
              ...
              <Button
                ...
                size="sm"
                className="h-7 gap-1 text-xs"
                onClick={() => addAllToWatchlist(list)}
              >
