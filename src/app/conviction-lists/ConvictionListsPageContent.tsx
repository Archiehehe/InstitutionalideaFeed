...
export function ConvictionListsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/conviction-lists?${searchParams.toString()}`)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 p-4 rounded-md border border-[#1F1F1F] bg-[#0A0A0A]">
        ...
        <Select value={filters.theme || ''} onValueChange={(v) => setFilter('theme', v || '')}>
        ...
        <Select value={filters.reviewStatus || ''} onValueChange={(v) => setFilter('reviewStatus', v || '')}>
        ...
        <Select value={filters.sourceType || ''} onValueChange={(v) => setFilter('sourceType', v || '')}>
        ...
        <Select value={filters.sortBy || 'updatedAt'} onValueChange={(v) => setFilter('sortBy', v || '')}>
        ...
        <Select value={filters.sortOrder || 'desc'} onValueChange={(v) => setFilter('sortOrder', v || '')}>
