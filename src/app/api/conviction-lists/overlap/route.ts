  ...
  try {
    ...
    if (lists.length === 0) {
      return Response.json({
        rows: [],
        count: 0,
        message: 'Import starter lists or use Sell-Side Finder to create lists first.',
        summary: {
          totalTickers: 0,
    ...
    for (const list of lists) {
      ...
      for (const member of members) {
        ...
        if (existing) {
          ...
          existing.actions.push(...(member.action ? [member.action.toLowerCase()] : []))
          ...
          existing.latestPeriod = latestPeriod(existing.latestPeriod, list.period)
        } else {
    ...
    const rows = Array.from(rowsByTicker.values())
      .map((row) => {
        const uniqueInstitutions = Array.from(new Set(row.institutions)).filter(Boolean)
        ...
        const uniqueReviewStatuses = Array.from(new Set(row.reviewStatuses)).filter(Boolean)
        const uniqueConfidences = Array.from(new Set(row.confidences)).filter(Boolean)
      ...
      .filter((row) => {
        if (search) {
          const haystack = `${row.ticker} ${row.companyName ?? ''}`.toLowerCase()
      ...
      .sort((a, b) => {
        ...
        if (b.institutionCount !== a.institutionCount) return b.institutionCount - a.institutionCount
    ...
    const summary = {
      ...
      verifiedMentions: rows.reduce((total, row) => total + row.lists.filter((list) => list.confidence === 'verified').length, 0),
      needsReviewMentions: rows.reduce((total, row) => total + row.lists.filter((list) => list.confidence === 'needs_review').length, 0),
    }
